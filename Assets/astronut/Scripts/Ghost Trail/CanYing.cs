...
public class CanYing : MonoBehaviour
{
    ...
    void OnEnable()
    {
        if (meshFilters == null || skinedMeshRenderers == null)
        {
            Initialize();
        }
    }
    ...
    private void Initialize()
    {
        ...
        if (meshFilters.Length == 0 && skinedMeshRenderers.Length == 0)
        {
            Debug.LogWarning("[CanYing] 找不到 MeshFilter 或 SkinnedMeshRenderer，无法生成残影。", this);
        }
    }
    ...
    void Update()
    {
        ...
        if (Time.time - lastCombinedTime > interval)
        {
            ...
            for (int i = 0; skinedMeshRenderers != null && i < skinedMeshRenderers.Length; ++i)
            {
                ...
                meshRenderer.sharedMaterials = skinedMeshRenderers[i].sharedMaterials;
